...
#define INVALID_FD (-1)
#define PORTNUM 5234
#define SERVADDR "172.17.0.2"
#define SERVADDR2 "172.17.0.3"
...
usage(void)
{
   ...
   fprintf(stderr, "\t-a, --agent <name>     name of the fast path agent to connect to\n");
   fprintf(stderr, "\t-h, --help             print this message\n");
   fprintf(stderr, "\t-l, --loglevel <level> set the loglevel\n");
   fprintf(stderr, "\t-d, --defaultaction <action> set the default action -eg. ACTION_COPY\n");
}
...
//New code starts 
   ...
   if(frameLen == 0)
     return;
   char buffer[2000];
   memcpy(buffer,pkt,frameLen);
   printf("after memcopy packet : %s",buffer);
   int sockfd, portno, n;
   ...
   localaddr.sin_port = htons(6234);  // Any  port will do
   bind(sockfd, (struct sockaddr *)&localaddr, sizeof(localaddr));

    if (sockfd < 0)
        error("ERROR opening socket");
    //server = gethostbyname(172.17.0.2]);
   // if (server == NULL) {
    ...
    serv_addr.sin_addr.s_addr = inet_addr(SERVADDR2);
    ...
    n = write(sockfd,buffer,frameLen);
    if (n < 0)
         error("ERROR writing to socket");
    bzero(buffer,2000);
    bzero(pkt,frameLen);
    shutdown(sockfd, SHUT_WR);
    n = read(sockfd,buffer,frameLen);
    if (n < 0)
         error("ERROR reading from socket");
    printf("Client got packet from container 1:%s\n",buffer);
    close(sockfd);
   // code to connect to server2
  if(1)
  {
    int sockfd2;
   ...
   localaddr.sin_addr.s_addr = inet_addr("172.17.0.1");
   localaddr.sin_port = htons(6234);  // Any  port will do
   bind(sockfd2, (struct sockaddr *)&localaddr, sizeof(localaddr));
   ...
   // if (server == NULL) {
    ...
    if (connect(sockfd2,(struct sockaddr *) &serv_addr,sizeof(serv_addr)) < 0)
      {
        printf("ERROR connecting");return ;
      }
    n = write(sockfd2,buffer,frameLen);
    if (n < 0)
         printf("ERROR writing to socket");
    bzero(buffer,2000);
    bzero(pkt,frameLen);
    shutdown(sockfd2, SHUT_WR);
    n = read(sockfd2,buffer,frameLen);
    if (n < 0)
         printf("ERROR reading from socket");
    printf("Client got packet from container 2:%s\n",buffer);
   close(sockfd2);
   }
    printf("Framelen:%d\n", frameLen);
    memcpy(pkt,buffer,frameLen);
    //close(sockfd2);
...
main(int argc, char *argv[])
{
   static DVFilterSlowPathOps ops = {
      ...
      .getSavedStateLen =     GetSavedStateLen,
      .saveState =            SaveState,
      .restoreState =         RestoreState,
      .processPacket =        ProcessPacket,
      .filterIoctlRequest =   ProcessFilterIoctlRequest,
      .slowPathIoctlRequest = ProcessSlowPathIoctlRequest,
   ...
   filterListCount = 0;
   filterListSize = 100;
   filterList = malloc(sizeof(filter*) * filterListSize);
   if (filterList == NULL) {
   ...
   static Netx_LibHandle_t netx_lib_hdl = {
      .Netx_LibReply_Callback = Netx_LibReply_Callback,
   };
